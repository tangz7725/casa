...
class ModelRunner:
    ...
    def __init__(
        ...
        # whh：读取基因组的数据
        self.gene_sequence = []
        self.current_gene_sequence = ''
        self.gene_filename = Path(gene_filename).resolve() if gene_filename else None
        if self.gene_filename:
            with open(self.gene_filename, 'r') as gene_file:
                for line in gene_file:
                    ...
                    if line.startswith('>'):
                        # 如果当前序列不为空，先将其加入结果列表
                        if self.current_gene_sequence:
                            self.gene_sequence.append(self.current_gene_sequence)
                            self.current_gene_sequence = ''  # 重置当前序列存储
                    else:
                        # 将序列行添加到当前序列
                        self.current_gene_sequence += line
    ...
    def predict(self, peak_path: Iterable[str], output: str) -> None:
        ...
        # whh：这里要记得把模型赋值，不然后续model会接收不到
        self.model.gene_sequence = self.gene_sequence
    ...
    def initialize_model(self, train: bool) -> None:
        ...
        model_params = dict(
            ...
            gene_sequence=self.gene_sequence,
        )
